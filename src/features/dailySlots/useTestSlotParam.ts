...
import { isSlotName, type SlotName } from "./slotUtils";

/**
 * `?test=morning|lunch|dinner` 쿼리 파라미터를 읽고 쓰는 훅입니다.
 * 실제 시계가 몇 시든 상관없이, 이 값이 있으면 그 시간대를 "현재"로 취급하도록
 * (slotUtils.ts의 resolveCurrentSlotIndex에서) 사용됩니다.
 *
 ...
 */
export function useTestSlotParam(): [SlotName | null, (value: SlotName | null) => void] {
  // 최초 렌더링 시 한 번만 실행되는 초기값 계산 함수(lazy initializer).
  // 현재 주소창의 ?test= 값을 읽어서, 유효한 슬롯 이름(morning/lunch/dinner)일 때만 사용합니다.
  const [testParam, setTestParamState] = useState<SlotName | null>(() => {
    const value = new URLSearchParams(window.location.search).get("test");
    return isSlotName(value) ? (value as SlotName) : null;
  });
  ...
  const setTestParam = useCallback((value: SlotName | null) => {
